...
LinkedinServer.get("/auth/linkedin/callback", async (req, res) => {
  ...
  try {
    ...
    const email = profileRes.data.email || `linkedin-${linkedInId}@noemail.com`;
    const photoUrl = profileRes.data.picture ?? "";
    ...
    try {
      ...
      user = await authAdmin.auth().getUser(uid);
      console.log("🔵 Existing Firebase user found:", user.uid);
    } catch (error) {
      if (error.code === "auth/user-not-found") {
        console.log("🟢 Creating new Firebase user...");
        user = await authAdmin.auth().createUser({
          uid: uid,
          email: email,
          emailVerified: true, // ✅ Bypassing email verification
          displayName: `${firstName} ${lastName}`,
          photoURL: photoUrl,
          providerData: [{ providerId: "linkedin.com" }]
        });
      } else {
        throw error;
      }
    ...
    const firebaseToken = await authAdmin.auth().createCustomToken(uid, {
      provider: "linkedin",
      email: email,
      displayName: `${firstName} ${lastName}`,
      profilePic: photoUrl,
